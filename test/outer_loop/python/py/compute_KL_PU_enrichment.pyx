...
cpdef inline compute_KL_PU_enrichment(
    ...
    '''
    Computes the KL_PU (antisymmetric Kullback-Leibler divergence) GSEA enrichment score for
    one gene set in one gene list
    '''
    
    cdef int i, n = len(S_h)
    cdef np.float_t eps = 1.0e-16
    cdef np.float_t ES, Delta_sum = 0, p_sum = eps, u_sum = eps
    ...
    cdef np.ndarray[np.float_t, ndim = 1] U_rev = np.empty(n, dtype = float)
    ...
    # Compute running enrichment: KL divergence between P and U minus same quantity from the reverse side
    
    for i in range(n):
        Delta[i] = P[i] * log((P[i] + eps)/(U[i] + eps)) - P_rev[i] * log((P_rev[i] + eps)/(U_rev[i] + eps))
        Delta_sum += Delta[i]
