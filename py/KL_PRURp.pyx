...
cpdef inline compute_KL_PRURp_enrichment(
    ...
    if alpha == 1.0:
        ...
        for i in range(n):
            if I_h[i] == 1:
                p_sum += abs(S_h[i]) + 1.0e-16
            else:
                u_sum += abs(S_h[i]) + 1.0e-16

            r_sum += abs(S_h[i]) + 1.0e-16
            p_cumsum[i] = p_sum   
            u_cumsum[i] = u_sum
            r_cumsum[i] = r_sum
        ...
        for i in range(n):
            if I_h[i] == 1:
                p_sum += pow(abs(S_h[i]), alpha) + 1.0e-16
            else:
                u_sum += pow(abs(S_h[i]), alpha) + 1.0e-16

            r_sum += pow(abs(S_h[i]), alpha) + 1.0e-16  
            p_cumsum[i] = p_sum
            u_cumsum[i] = u_sum
            r_cumsum[i] = r_sum
